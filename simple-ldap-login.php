...
class SimpleLDAPLogin {
	...
	public static function get_field_settings_s() {
		return self::$prefix_s . 'settings';
	}

	public function __construct() {
		$this->prefix   = self::$prefix_s;
		$this->settings = $this->get_settings_obj();

		if ( trim( $this->get_setting( 'directory' ) ) === 'ad' ) {
			require_once plugin_dir_path( __FILE__ ) . '/lib/adLDAP.php';

			try {
				try {
					$this->create_adldap();
					$this->bind_user_success = true;
				} catch ( adLDAPException $e ) {
					// try to create adldap again.
					$this->create_adldap( true );
					$this->bind_user_success = false;
				}
			} catch ( adLDAPUnavailableServerException $e ) {
				$this->domain_controler_unavailable = true;
				error_log( "Simple LDAP Login warning: {$e->getMessage()}" ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			}
		}
		...
		if ( sldap_str_true( $this->get_setting( 'enabled' ) ) ) {
		...
		if ( sldap_str_true( $this->get_setting( 'sso_enabled' ) ) ) {
			add_action( 'init', array( $this, 'login_sso' ) );
		}
	}

	private function create_adldap( $ignore_user = false ) {
		$this->adldap = new adLDAP(
			array(
				'account_suffix'     => trim( $this->get_setting( 'account_suffix' ) ),
				'use_tls'            => sldap_str_true( $this->get_setting( 'use_tls' ) ),
				...
				'ad_username'        => ! $ignore_user && $this->is_sso_enabled() ? $this->get_setting( 'sso_search_user' ) : null,
				'ad_password'        => ! $ignore_user && $this->is_sso_enabled() ? $this->get_sso_search_user_pass() : null,
			)
		);
	}
	...
	private function is_valid_call_sso() {
		...
		// Skip login page.
		if ( wp_parse_url( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ), PHP_URL_PATH ) === '/wp-login.php' ) {
			return false;
		}
	...
	public function login_sso() {
		...
		if ( ! is_user_logged_in() && $this->is_sso_configuration_ok() && $this->is_valid_call_sso() && $this->bind_user_success ) {
			// Automatic login.
			...
			do_action( 'wp_login', $usu->user_login, $usu ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP core action.
		}
	}
	...
	private function upgrade_settings() {
		if ( $this->get_setting( 'version' ) === false ) {
			...
			$this->set_setting( 'use_tls', sldap_str_true( $simpleldap_use_tls ) );
	...
	private function get_settings_obj() {
		if ( $this->is_network_version() ) {
			return get_site_option( self::get_field_settings_s(), false );
		} else {
			return get_option( self::get_field_settings_s(), false );
		}
	}

	private function set_settings_obj( $newobj ) {
		if ( $this->is_network_version() ) {
			return update_site_option( self::get_field_settings_s(), $newobj );
		} else {
			return update_option( self::get_field_settings_s(), $newobj );
		}
	}

	private function set_setting( $option, $newvalue ) {
		if ( $option === false ) {
			return false;
		}
	...
	private function get_setting( $option = false ) {
	...
	private function add_setting( $option, $newvalue ) {
		if ( $option === false ) {
			return false;
		}
	...
	public function saved_admin_notice() {
		if ( sldap_str_true( $this->get_setting( 'enabled' ) ) ) {
			?>
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e( 'Simple LDAP Login settings have been saved.', 'simple-ldap-login' ); ?></p>
			</div>
			<?php
		} else {
			?>
			<div class="notice notice-error is-dismissible">
				<p><?php esc_html_e( 'Simple LDAP Login is disabled.', 'simple-ldap-login' ); ?></p>
			</div>
			<?php
		}
	}

	public function authenticate( $user, $username, $password, $sso_auth = false ) {
		...
		$local_admin = apply_filters( $this->prefix . 'force_ldap', $local_admin );
		...
		if ( empty( $username ) || empty( $password ) ) {
			...
			if ( empty( $username ) ) {
				$error->add( 'empty_username', __( '<strong>ERROR</strong>: The username field is empty.', 'simple-ldap-login' ) );
			}

			if ( empty( $password ) ) {
				$error->add( 'empty_password', __( '<strong>ERROR</strong>: The password field is empty.', 'simple-ldap-login' ) );
			}
		...
		if ( apply_filters( $this->prefix . 'remove_default_authentication_hook', sldap_str_true( $this->get_setting( 'high_security' ) ) && ! $local_admin ) ) {
			remove_filter( 'authenticate', 'wp_authenticate_username_password' );
			remove_filter( 'authenticate', 'wp_authenticate_email_password' );
			remove_filter( 'authenticate', 'wp_authenticate_application_password' );
		}
		...
		try {
			...
			return $this->ldap_auth_error(
				"{$this->prefix}login_error",
				__(
					'<strong>Simple LDAP Login</strong>: no Domain Controller Available.',
					'simple-ldap-login'
				)
			);
		}
		...
		if ( $auth_result ) {
			...
			if ( $this->user_has_groups( $username, trim( $this->get_setting( 'directory' ) ) ) ) {
				...
				if ( ! $user || ( strtolower( $user->user_login ) !== strtolower( $username ) ) ) {
					if ( ! sldap_str_true( $this->get_setting( 'create_users' ) ) ) {
						do_action( 'wp_login_failed', $username ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP core hook.
						return $this->ldap_auth_error(
							'invalid_username',
							__(
								'<strong>Simple LDAP Login Error</strong>: LDAP credentials are correct, but there is no matching WordPress user and user creation is not enabled.',
								'simple-ldap-login'
							)
						);
					}
					...
					if ( ! is_wp_error( $new_user ) ) {
						...
						return $this->ldap_auth_error(
							...
							// translators: %s: error message.
								__( '<strong>Simple LDAP Login Error</strong>: LDAP credentials are correct, but there is no matching WordPress user and user creation failed with the following error: %s', 'simple-ldap-login' ),
				...
				return $this->ldap_auth_error(
					"{$this->prefix}login_error",
					__(
						'<strong>Simple LDAP Login Error</strong>: Your LDAP credentials are correct, but you are not in an authorized LDAP group.',
						'simple-ldap-login'
					)
				);
			}
		} elseif ( sldap_str_true( $this->get_setting( 'high_security' ) ) ) {
			return $this->ldap_auth_error(
				'invalid_username',
				__(
					'<strong>Simple LDAP Login</strong>: Simple LDAP Login could not authenticate your credentials. The security settings do not permit trying the WordPress user database as a fallback.',
					'simple-ldap-login'
				)
			);
		}
	...
	private function get_domain_username( $username ) {
	...
	private function ldap_auth( $username, $password, $directory, $sso_auth ) {
		$result = false;

		if ( $directory === 'ad' ) {
			if ( $this->domain_controler_unavailable ) {
			...
			if ( sldap_str_true( $this->get_setting( 'search_sub_ous' ) ) ) {
	 ...
	 * @return WP_Error
	 */
	private function ldap_auth_error( $code, $message ) {
		if ( sldap_str_true( $this->get_setting( 'high_security' ) ) ) {
	...
	private function user_has_groups( $username, $directory ) {
		$result = false;
	...
	private function get_user_data( $username, $directory, $update = false ) {
		if ( ! $update ) {
			$user_data = array(
		...
		if ( $directory === 'ad' ) {
			$userinfo = $this->adldap->user_info(
				$username,
				array(
	...
	private function get_user_meta_data( $username, $directory ) {
	...
	private function meta_data_filter( $value, $type ) {
		if ( $type === 'number' ) {
	...
	private function get_meta_key_ldap( $meta_key ) {
		return "{$meta_key}_{$this->get_setting('meta_data_suffix_ldap')}";
	}

	private function get_meta_key_wp( $meta_key ) {
	 ...
	 */
	private function is_network_version() {
	...
	/**
	 * Cleanup ldap filter values.
	 *
	 * @param string $str The string to escape.
	 *
	 * @return string The escaped string.
	 */
	private function esc_ldap_filter_val( $str = '' ) {
		if ( function_exists( 'ldap_escape' ) ) {
			return ldap_escape( $str, '', LDAP_ESCAPE_FILTER );
		}

		$meta_chars        = array( '\\', "\00", '(', ')', '*' );
		$quoted_meta_chars = array( "\\0x5x", "\\0x00", "\\0x28", "\\0x29", "\\0x2a" );
		$str               = str_replace( $meta_chars, $quoted_meta_chars, $str );

		return $str;
	}

	private function is_sso_enabled() {
		return sldap_str_true( $this->get_setting( 'sso_enabled' ) );
	}

	private function is_sso_configuration_ok() {
		return isset( $_SERVER['REMOTE_USER'] ) && ! empty( $_SERVER['REMOTE_USER'] );
	}

	/**
	 * Returns the username of the logged user or false if not logged.
	 *
	 * @return string|bool
	 */
	private function get_sso_logged_user() {
		if ( ! $this->is_sso_configuration_ok() ) {
			return false;
		}
	...
	private function sso_search_user_bind_test() {
	...
	private function get_sso_search_user_pass() {
		$crypt_pass = explode( ';', $this->get_setting( 'sso_search_user_password' ) );
		if ( count( $crypt_pass ) !== 2 ) {
			return ''; // wrong pass.
		}
		return openssl_decrypt( $crypt_pass[0], self::$openssl_method, $this->get_openssl_pass(), 0, hex2bin( $crypt_pass[1] ) );
	}
 ...
 * @return bool
 **/
function sldap_str_true( $string, $is_true = array( 'yes', 'y', 'true', '1', 'on', 'open' ) ) {
	if ( is_array( $string ) ) {
		return false;
	}
	...
	return in_array( strtolower( $string ), $is_true, true );
}

$simple_ldap_login = SimpleLDAPLogin::get_instance();
