...
class adLDAP {
	 ...
	 * @param bool optional $sso_bind Set true when it's a SSO bind. Default: false
	 * @return bool
	 */
	public function authenticate( $username, $password, $prevent_rebind = false, $sso_bind = false ) {
	...
	public function exchange_contact_mailenable( $distinguishedname, $emailaddress, $mailnickname = null ) {
		...
		$attributes = array(
			'email'                 => $emailaddress,
			'contact_email'         => 'SMTP:' . $emailaddress,
			'exchange_proxyaddress' => 'SMTP:' . $emailaddress,
			'exchange_mailnickname' => $mailnickname,
		);
